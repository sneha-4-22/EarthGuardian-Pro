...
const Cf1  = () => {
  ...
  const [coalPercent, setCoalPercent] = useState(0);
  const [gasPercent, setGasPercent] = useState(0);
  const [oilPercent, setOilPercent] = useState(0);
  const [hydroPercent, setHydroPercent] = useState(0);
  const [renewablePercent, setRenewablePercent] = useState(0);
  const [nuclearPercent, setNuclearPercent] = useState(0);
  const [customPercent, setCustomPercent] = useState(0);
  const [data, setData] = useState(null); // Store the loaded data
  ...
  const calculateCO2Emissions = () => {
    if (data && selectedCountry) {
      // Calculate CO2 emissions based on user inputs and loaded data
      const countryData = data[selectedCountry];
      const coal_CO2 = countryData.Coal;
      const gas_CO2 = countryData.Gas;
      const oil_CO2 = countryData.Oil;
      const hydro_CO2 = countryData.Hydro;
      const renewable_CO2 = countryData.Renewable;
      const nuclear_CO2 = countryData.Nuclear;
      const custom_CO2 = 0; // Set the custom value

      const calculatedOverallEmission =
        coalPercent + gasPercent + oilPercent + hydroPercent + renewablePercent + nuclearPercent + customPercent;

      const calculatedKgCO2Result =
        ((coalPercent * coal_CO2 + gasPercent * gas_CO2 + oilPercent * oil_CO2 +
          hydroPercent * hydro_CO2 + renewablePercent * renewable_CO2 +
          nuclearPercent * nuclear_CO2 + customPercent * custom_CO2) /
          100000).toFixed(2);

      setOverallEmission(calculatedOverallEmission);
      setKgCO2Result(calculatedKgCO2Result);
    }
  };
  ...
  const countryNames = Object.keys(parsedData);

  return (
    <View>
      <Text style={{ fontSize: 24, textAlign: 'center' }}>Carbon Footprint Calculator</Text>
      ...
      <Slider value={coalPercent} onValueChange={(value) => setCoalPercent(value)} />
      <Slider value={gasPercent} onValueChange={(value) => setGasPercent(value)} />
      {/* Add sliders for other energy sources similarly */}
      <Slider value={oilPercent} onValueChange={(value) => setOilPercent(value)} />
      <Slider value={hydroPercent} onValueChange={(value) => setHydroPercent(value)} />
      <Slider value={renewablePercent} onValueChange={(value) => setRenewablePercent(value)} />
      <Slider value={nuclearPercent} onValueChange={(value) => setNuclearPercent(value)} />
      <Slider value={customPercent} onValueChange={(value) => setCustomPercent(value)} />

      {/* Calculate CO2 Emissions */}
      <Button title="Calculate" onPress={calculateCO2Emissions} />

      {/* Display Results */}
      <Text>Overall Emission %: {OverallEmission}</Text>
      <Text>CO2 Emissions (tons): {kgCO2Result} tons of CO2</Text>
    </View>
  );
}
export default Cf1;